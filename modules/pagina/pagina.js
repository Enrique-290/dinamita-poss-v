    ...
    refs.importJsonFile?.addEventListener('change', async (e)=>{
      try{
        ...
        const importedState = payload && typeof payload === 'object'
          ? (payload.state || payload.pageState || payload.page || payload.config || payload)
          : null;
...
function renderCarrito(){
  ...
  function buildExportHtml(){
    const exportState = mergeDeep(clone(DEFAULT_STATE), clone(state || {}));
    const exportProducts = getTPVProducts();
    const exportPayload = {
      state: exportState,
      products: exportProducts
    };
    const safeJson = JSON.stringify(exportPayload)
      .replace(/</g, '\u003c')
      .replace(/>/g, '\u003e')
      .replace(/&/g, '\u0026');
...
<head>
  ...
  <title>${escapeHtml(exportState.general.businessName || 'Página')}</title>
  <style>
    *{box-sizing:border-box}body{margin:0;font-family:Arial,Helvetica,sans-serif;background:#f6f6f6;color:#111}img{max-width:100%;display:block}
    ...
    .header{display:flex;justify-content:space-between;align-items:center;gap:12px;padding:18px;background:#fff;border:1px solid #ececec;border-radius:20px;margin:16px 0}.brand{display:flex;gap:12px;align-items:center}
    .logo{width:64px;height:64px;border-radius:18px;background:#c00000;color:#fff;display:flex;align-items:center;justify-content:center;font-weight:900;font-size:26px}
    .muted{color:#666}.btn{display:inline-flex;align-items:center;justify-content:center;padding:10px 16px;border-radius:999px;border:1px solid #ddd;background:#fff;color:#222;font-weight:800;text-decoration:none;cursor:pointer}
    .btn.primary{background:#c00000;border-color:#c00000;color:#fff}.btn.whatsapp{background:#25D366;border-color:#25D366;color:#fff}.btn:hover{filter:brightness(.97)}
    .hero{display:grid;grid-template-columns:1.1fr .9fr;gap:16px;align-items:stretch}.heroMedia{min-height:260px;border-radius:18px;background:#eee center/cover no-repeat;display:flex;align-items:center;justify-content:center;font-weight:800;color:#666;overflow:hidden}
    ...
    .product{border:1px solid #ececec;border-radius:18px;padding:14px;background:#fff}.pmedia{position:relative;border-radius:14px;background:#f1f1f1;min-height:160px;display:flex;align-items:center;justify-content:center;overflow:hidden}
    .pmedia img{width:100%;height:160px;object-fit:cover}.fallback{font-size:40px;font-weight:900;color:#999}.tag{position:absolute;top:10px;right:10px;padding:6px 10px;border-radius:999px;background:#fff3f3;color:#b30000;font-size:12px;font-weight:800}.tag.new{background:#eef8ee;color:#0f7b27}
    .cat{display:inline-block;padding:6px 10px;border-radius:999px;background:#f3f3f3;color:#666;font-size:12px;font-weight:800;margin:12px 0 8px}.price{color:#c00000;font-size:36px;font-weight:900;margin:8px 0}.small{font-size:13px;color:#666}
    .pactions{display:flex;gap:8px;flex-wrap:wrap;margin-top:10px}.toolbar{display:flex;gap:12px;justify-content:space-between;align-items:center;flex-wrap:wrap}.search{padding:12px 14px;border:1px solid #ddd;border-radius:14px;min-width:260px;flex:1}
    ...
    .empty{padding:16px;border:1px dashed #ddd;border-radius:16px;background:#fafafa;color:#666}.floatingWa{position:fixed;right:18px;bottom:18px;z-index:30}
    .secondaryBanner{margin-top:16px}.secondaryBanner .heroMedia{min-height:180px}
    ...
    @media (max-width: 860px){.hero{grid-template-columns:1fr}.hero h1{font-size:42px}.contact{grid-template-columns:1fr}.cartItem{grid-template-columns:1fr}.header{flex-direction:column;align-items:flex-start}.detailCard{grid-template-columns:1fr}}
  </style>
</head>
<body>
  <div id="app"></div>
  <script id="page2-data" type="application/json">${safeJson}</script>
  <script>
    (function(){
      const payload = JSON.parse(document.getElementById('page2-data').textContent || '{}');
      const pageState = payload.state || {};
      const products = Array.isArray(payload.products) ? payload.products : [];
      if(!pageState.ui) pageState.ui = { search:'', selectedCategory:'all', detailId:'' };
      if(!pageState.cart) pageState.cart = { items: [] };
      ...
      function waNumber(){ return normalizeMxNumber((pageState.contacto && pageState.contacto.whatsapp) || (pageState.contacto && pageState.contacto.phone) || ''); }
      function openWa(message){ const number = waNumber(); if(!number){ alert('Captura primero WhatsApp o teléfono en Contacto.'); return; } const url = 'https://api.whatsapp.com/send?phone=' + number + '&text=' + encodeURIComponent(message); window.open(url, '_blank', 'noopener,noreferrer'); }
      function productMessage(p){ return 'Hola, me interesa:\n' + p.name + '\nPrecio: ' + money(p.price) + '\nCategoría: ' + (p.category || 'General'); }
      function categories(){ const fromProducts = [...new Set(products.map(p => p.category).filter(Boolean))]; const manual = Array.isArray(pageState.categorias && pageState.categorias.items) ? pageState.categorias.items : []; return [...new Set([...fromProducts, ...manual])]; }
      function selectedCategory(){ return pageState.ui && pageState.ui.selectedCategory ? pageState.ui.selectedCategory : 'all'; }
      function filteredProducts(){ const q = String(pageState.ui && pageState.ui.search || '').trim().toLowerCase(); return products.filter(p => { const catOk = selectedCategory() === 'all' || p.category === selectedCategory(); const qOk = !q || String(p.name||'').toLowerCase().includes(q) || String(p.category||'').toLowerCase().includes(q) || String(p.sku||'').toLowerCase().includes(q); return catOk && qOk; }).slice(0, Number(pageState.catalogo && pageState.catalogo.catalogLimit || 12)); }
      function featured(){ return products.slice(0, Number(pageState.catalogo && pageState.catalogo.featuredLimit || 6)); }
      function mostSold(){ return products.slice(0, Number(pageState.catalogo && pageState.catalogo.mostSoldLimit || 6)); }
      function newest(){ return products.slice().reverse().slice(0, Number(pageState.catalogo && pageState.catalogo.newLimit || 6)); }
      function findProduct(id){ return products.find(p => String(p.id) === String(id)); }
      function cartItems(){ return Array.isArray(pageState.cart && pageState.cart.items) ? pageState.cart.items : []; }
      function addToCart(id){ const p = findProduct(id); if(!p) return; const items = cartItems(); const ex = items.find(x => String(x.id) === String(id)); if(ex){ ex.qty += 1; } else { items.push({ id:p.id, name:p.name, price:Number(p.price||0), category:p.category||'General', qty:1 }); } render(); }
      function updateQty(id, delta){ const items = cartItems(); const it = items.find(x => String(x.id) === String(id)); if(!it) return; it.qty += delta; if(it.qty <= 0){ removeFromCart(id); return; } render(); }
      function removeFromCart(id){ pageState.cart.items = cartItems().filter(x => String(x.id) !== String(id)); render(); }
      function clearCart(){ pageState.cart.items = []; render(); }
      function cartTotal(){ return cartItems().reduce((sum, item) => sum + Number(item.price||0) * Number(item.qty||0), 0); }
      ...
      function productCard(p,badge){ const media = p.image ? '<img src="' + esc(p.image) + '" alt="' + esc(p.name) + '">' : '<div class="fallback">' + esc(initials(p.name)) + '</div>'; const tag = badge === 'top' ? '<span class="tag">Más vendido</span>' : badge === 'new' ? '<span class="tag new">Nuevo</span>' : ''; return '<article class="product"><div class="pmedia">' + media + tag + '</div><div class="cat">' + esc(p.category || 'General') + '</div><h3 style="margin:0 0 8px">' + esc(p.name) + '</h3><div class="small">Stock: ' + Number(p.stock||0) + '</div><div class="price">' + money(p.price) + '</div><div class="pactions"><button class="btn" data-action="detail" data-id="' + esc(p.id) + '">Ver detalle</button><button class="btn" data-action="add" data-id="' + esc(p.id) + '">Agregar</button><button class="btn whatsapp" data-action="wa" data-id="' + esc(p.id) + '">WhatsApp</button></div></article>'; }
      function secondaryBanner(){ if(!(pageState.banners && pageState.banners.secondaryImage)) return ''; return '<section class="section secondaryBanner"><div class="toolbar"><div><h2 style="margin:0">Banner secundario</h2><div class="muted">Imagen promocional complementaria</div></div></div><div class="heroMedia" style="background-image:url('' + esc(pageState.banners.secondaryImage) + '')"></div></section>'; }
      function cartSection(){ const items = cartItems(); return '<section class="section cart"><div class="toolbar"><div><h2 style="margin:0">Carrito</h2><div class="muted">Pedido rápido por WhatsApp</div></div></div><div class="cartWrap" style="margin-top:12px">' + (items.length ? '<div class="cartList">' + items.map(i => '<div class="cartItem"><div><strong>' + esc(i.name) + '</strong><div class="small">' + esc(i.category||'General') + ' · ' + money(i.price) + '</div></div><div class="cartControls"><button class="btn" data-action="minus" data-id="' + esc(i.id) + '">-</button><span class="qty">' + i.qty + '</span><button class="btn" data-action="plus" data-id="' + esc(i.id) + '">+</button><button class="btn" data-action="remove" data-id="' + esc(i.id) + '">Quitar</button></div><div><strong>' + money(Number(i.price||0)*Number(i.qty||0)) + '</strong></div></div>').join('') + '</div><div class="cartFooter"><div>Total: <strong>' + money(cartTotal()) + '</strong></div><div style="display:flex;gap:8px;flex-wrap:wrap"><button class="btn" data-action="clear">Vaciar</button><button class="btn whatsapp" data-action="send-cart">Enviar por WhatsApp</button></div></div>' : '<div class="empty">Tu carrito está vacío. Agrega productos desde el catálogo.</div>') + '</div></section>'; }
      function contactSection(){ const socials = [pageState.contacto && pageState.contacto.facebook ? '<a class="btn" href="' + esc(pageState.contacto.facebook) + '" target="_blank" rel="noopener">Facebook</a>' : '', pageState.contacto && pageState.contacto.instagram ? '<a class="btn" href="' + esc(pageState.contacto.instagram) + '" target="_blank" rel="noopener">Instagram</a>' : ''].join(''); return '<section class="section" id="contacto"><div class="toolbar"><div><h2 style="margin:0">Contacto</h2><div class="muted">Canales directos de atención</div></div></div><div class="contact" style="margin-top:12px"><div class="contactGrid"><div class="contactItem"><strong>Dirección</strong><div class="muted">' + esc(pageState.contacto && pageState.contacto.address || 'Agrega la dirección del gym.') + '</div></div><div class="contactItem"><strong>Teléfono</strong><div class="muted">' + esc(pageState.contacto && pageState.contacto.phone || 'Agrega tu teléfono visible.') + '</div></div><div class="contactItem"><strong>WhatsApp</strong><div class="muted">' + esc(pageState.contacto && pageState.contacto.whatsapp || 'Pendiente') + '</div></div><div class="contactItem"><strong>Redes</strong><div class="muted">' + esc([pageState.contacto && pageState.contacto.facebook?'Facebook':'', pageState.contacto && pageState.contacto.instagram?'Instagram':''].filter(Boolean).join(' · ') || 'Pendiente') + '</div></div></div><div style="display:flex;flex-direction:column;gap:10px"><button class="btn primary" data-action="wa-contact">Escríbenos por WhatsApp</button>' + socials + '</div></div></section>'; }
      function detailOverlay(){ if(!(pageState.ui && pageState.ui.detailId)) return ''; const p = findProduct(pageState.ui.detailId); if(!p) return ''; const media = p.image ? '<img src="' + esc(p.image) + '" alt="' + esc(p.name) + '">' : '<div class="fallback">' + esc(initials(p.name)) + '</div>'; return '<div class="detailOverlay open" data-action="close-detail"><div class="detailCard" onclick="event.stopPropagation()" style="position:relative"><button class="btn detailClose" data-action="close-detail">Cerrar</button><div class="detailMedia">' + media + '</div><div><div class="cat">' + esc(p.category||'General') + '</div><h2>' + esc(p.name) + '</h2><div class="price">' + money(p.price) + '</div><div class="small">Stock: ' + Number(p.stock||0) + '</div><p class="muted">SKU: ' + esc(p.sku || 'Sin SKU') + '</p><div class="actions"><button class="btn" data-action="add" data-id="' + esc(p.id) + '">Agregar al carrito</button><button class="btn primary" data-action="wa" data-id="' + esc(p.id) + '">Comprar por WhatsApp</button></div></div></div></div>'; }
      function render(){ document.getElementById('app').innerHTML = '<div class="wrap">' + '<header class="header"><div class="brand"><div class="logo">' + esc(initials(pageState.general && pageState.general.businessName)) + '</div><div><div style="font-size:28px;font-weight:900">' + esc(pageState.general && pageState.general.businessName || 'Dinamita Gym') + '</div><div class="muted">' + esc(pageState.general && pageState.general.tagline || '') + '</div></div></div><div style="display:flex;gap:8px;flex-wrap:wrap"><button class="btn" data-action="scroll-contact">Contacto</button><button class="btn primary" data-action="wa-hero">Entrenar ahora</button></div></header>' + '<section class="section"><div class="hero"><div><h1>' + esc(pageState.general && pageState.general.tagline || 'Explota tu potencial') + '</h1><p>' + esc(pageState.general && pageState.general.welcomeText || '') + '</p><div class="actions"><button class="btn primary" data-action="wa-hero">Entrenar ahora</button><button class="btn" data-action="scroll-contact">Ver contacto</button></div></div><div class="heroMedia" style="' + ((pageState.banners && pageState.banners.heroImage) ? 'background-image:url('' + esc(pageState.banners.heroImage) + '')' : '') + '">' + ((pageState.banners && pageState.banners.heroImage) ? '' : 'Banner principal') + '</div></div></section>' + secondaryBanner() + '<section class="section"><div class="toolbar"><div><h2 style="margin:0">Categorías</h2><div class="muted">Filtra el catálogo por tipo de producto</div></div></div><div class="chips" style="margin-top:12px"><button class="chip ' + (selectedCategory()==='all'?'active':'') + '" data-action="cat" data-cat="all">Todo</button>' + categories().map(c => '<button class="chip ' + (selectedCategory()===c?'active':'') + '" data-action="cat" data-cat="' + esc(c) + '">' + esc(c) + '</button>').join('') + '</div></section>' + '<section class="section"><div class="toolbar"><div><h2 style="margin:0">Productos destacados</h2><div class="muted">Selección principal del catálogo</div></div></div><div class="grid featured" style="margin-top:12px">' + (featured().map(p => productCard(p)).join('') || '<div class="empty">No hay productos.</div>') + '</div></section>' + '<section class="section"><div class="toolbar"><div><h2 style="margin:0">Lo más vendido</h2><div class="muted">Productos con mayor salida</div></div></div><div class="grid" style="margin-top:12px">' + (mostSold().map(p => productCard(p, 'top')).join('') || '<div class="empty">No hay productos.</div>') + '</div></section>' + '<section class="section"><div class="toolbar"><div><h2 style="margin:0">Nuevos productos</h2><div class="muted">Lo más reciente del catálogo</div></div></div><div class="grid" style="margin-top:12px">' + (newest().map(p => productCard(p, 'new')).join('') || '<div class="empty">No hay productos.</div>') + '</div></section>' + '<section class="section"><div class="toolbar"><div><h2 style="margin:0">Catálogo</h2><div class="muted">Vista conectada a tu inventario</div></div><input class="search" placeholder="Buscar producto por nombre, categoría o SKU" value="' + esc(pageState.ui && pageState.ui.search || '') + '" data-action="search"></div><div class="muted" style="margin-top:10px">Mostrando ' + filteredProducts().length + ' de ' + products.length + ' producto(s)</div><div class="grid featured" style="margin-top:14px">' + (filteredProducts().map(p => productCard(p)).join('') || '<div class="empty">No hay resultados con este filtro.</div>') + '</div></section>' + cartSection() + contactSection() + '<footer class="footer">Página 2.0 exportada · Dinamita POS</footer>' + '<button class="btn whatsapp floatingWa" data-action="wa-contact">WhatsApp</button>' + detailOverlay() + '</div>'; }
      document.addEventListener('click', function(e){ const btn = e.target.closest('[data-action]'); if(!btn) return; const action = btn.getAttribute('data-action'); const id = btn.getAttribute('data-id'); const cat = btn.getAttribute('data-cat'); if(action === 'cat'){ pageState.ui.selectedCategory = cat || 'all'; render(); return; } if(action === 'add' && id){ addToCart(id); return; } if(action === 'plus' && id){ updateQty(id, 1); return; } if(action === 'minus' && id){ updateQty(id, -1); return; } if(action === 'remove' && id){ removeFromCart(id); return; } if(action === 'clear'){ clearCart(); return; } if(action === 'send-cart'){ openWa(cartMessage()); return; } if(action === 'wa-hero' || action === 'wa-contact'){ openWa('Hola, quiero información del gym y sus productos.'); return; } if(action === 'scroll-contact'){ var target = document.getElementById('contacto'); if(target) target.scrollIntoView({behavior:'smooth'}); return; } if(action === 'wa' && id){ const p = findProduct(id); if(p) openWa(productMessage(p)); return; } if(action === 'detail' && id){ pageState.ui.detailId = id; render(); return; } if(action === 'close-detail'){ pageState.ui.detailId = ''; render(); return; } });
      document.addEventListener('input', function(e){ if(e.target && e.target.getAttribute('data-action') === 'search'){ pageState.ui.search = e.target.value || ''; render(); } });
      render();
    })();
  <\/script>
</body>
